...
function downloadFile(url, outputFilename, urlIdx) {
    "use strict";
...
function downloadFiles(urlList, baseOutputPath) {
    ...
    urlList.forEach(function (curUrl) {
        ...
        if (promise === null) {
            promise = downloadFile(curUrl, destPath, curIdx);
        } else {
            var cb = function (val) {
                var newIdx = val + 1,
                    newDestPath = baseOutputPath + path.sep + path.basename(urlList[newIdx]);
                return downloadFile(urlList[newIdx], newDestPath, newIdx);
            };
    ...
    q.when(promise).then(function (val) {
        console.log("OKK!!!!!!!!!!!!".white);
